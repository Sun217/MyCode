...
def cut_patch(mb_img):#for numpy
    ...
    bottom = top + random.randrange(round(h * scale_min), round(h * scale_max))
    left = random.randrange(0, round(w))
    # right = left + random.randrange(round(h * 0.8), round(h * 0.9))
    right = left + random.randrange(round(h * scale_min), round(h * scale_max))
    if (bottom - top) % 2 == 1:
        bottom -= 1
    if (right - left) % 2 == 1:
        right -= 1
...
def paste_patch(img, patch):
    ...
    patch_w_position:patch_w_position + patchw, :] = patch
